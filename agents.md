...
- **`requirements.txt`** (raíz, nuevo 2026-08-15) — no existía; las dependencias se instalaban
  ...
  reemplazado por Modal) ni `FlagEmbedding` (corre dentro del contenedor de Modal). Hallazgo de
  paso: `server.py` (KM legacy MCP server) importa `mcp`, que no está instalado — roto tal cual,
  sin resolver.
