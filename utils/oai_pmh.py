"""
Harvester OAI-PMH genérico.

Cosecha metadata de repositorios académicos que implementen el protocolo OAI-PMH 2.0.
Soporta Dublin Core (oai_dc) como formato base.

Movido de knowledge_module/connectors/oai_pmh.py el 2026-08-14: era código genérico sin
hardcodeo de CRIZA, pero su único consumidor real (harvest_conicet.py) es de CRIZA — bajo el
principio de "cada instancia sus herramientas", baja acá en vez de vivir en el paquete
compartido.

Uso:
    from utils.oai_pmh import OAIPMHHarvester
...
class OAIPMHHarvester:
    ...
    La config de endpoint, sets y filtros viene del caller.
    """
