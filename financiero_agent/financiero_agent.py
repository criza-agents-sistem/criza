...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "MODERADAMENTE_VARIABLE/NO_ESTABLE. Usar para evaluar si un costo/precio/serie\n"
            "económica es consistente en el tiempo antes de usarla en el modelo. Nunca calcules\n"
            "esto vos mismo leyendo la tabla."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Detecta si una serie económica cambió de nivel de forma real y sostenida (no ruido\n"
            "de un solo punto) — ej. si un costo/precio se movió a un nuevo régimen en algún\n"
            "momento del período disponible. Un CV% sobre todo el período puede esconder esto."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Correlación real con desfase temporal entre dos series (ej. un costo insumo vs. un\n"
            "precio de venta varias semanas después). SIEMPRE incluye un chequeo de robustez\n"
            "(series diferenciadas) — si la correlación no lo pasa, vuelve marcada\n"
            "'robusta: false': puede ser una coincidencia de tendencias compartidas, no una\n"
            "relación real. Nunca reportes una correlación como hallazgo real sin mirar ese campo."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Compara dos series que deberían medir lo mismo (ej. dos fuentes de un mismo precio\n"
            "o índice) antes de combinarlas — detecta sesgo sistemático (razón constante entre\n"
            "ambas) en vez de asumir que son intercambiables."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "previsualización de ese documento (título/contenido en tu input)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
- analizar_estabilidad_serie / detectar_cambios_de_regimen / correlacion_con_desfase /
  comparar_fuentes_de_datos: cómputo numérico REAL (no estimado a ojo) sobre series de costos/
  precios/indicadores. Nunca calcules una media, un CV% o una correlación vos mismo leyendo una
  tabla — el resultado no es confiable. correlacion_con_desfase siempre incluye un chequeo de
...
- leer_serie_de_documento_aportado: trae una serie real [{'fecha','valor'}] directo de un Excel
  que Sebas aportó — usala antes de pedirle los números a mano si el documento ya está en la
  lista de "Documentos aportados" de tu input.
...
async def _dispatch(name: str, inputs: dict, *, caso_id: str, verbose: bool) -> str:
    if name == "ver_informe_especialista":
        ...
        documento_id = inputs.get("documento_id", "")
        if verbose:
            print(f"  -> leer_serie_de_documento_aportado: {documento_id} hoja={inputs.get('hoja')}")
