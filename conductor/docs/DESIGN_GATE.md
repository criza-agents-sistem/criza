...
| **Captura de qué se decidió en la conversación (destilado, no raw)** | Ej. "descartamos microalgas por costo de escalado" como una lección reusable, no como texto crudo de chat | área `lecciones` (`knowledge_module/aprendizaje.py`, SEB-156) | — | 🔵 **no resuelto** — Etapa 9 del plan de construcción. El historial raw (fila de arriba) ya sobrevive un reinicio, pero nadie todavía destila ese historial en una lección buscable/reusable — son dos problemas distintos, no el mismo gap. |
