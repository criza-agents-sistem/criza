...
| J | Etapa 19 (2026-08-19) — Sebas: "necesito asignarle al Conductor la función de crear documentos" — hoy solo podía crear documentos indirectamente, invocando a un especialista. ¿Cuándo dispara, siempre atado a un frente, y quién redacta el contenido? | Ver detalle abajo | **Ambos triggers** (pedido explícito → guarda directo; detecta algo consolidable sin que se lo pidan → propone título+resumen y espera confirmación, mismo patrón que `crear_caso`), **siempre atado a un caso/frente** (consistente con `documento_caso`/`documento_aportado`), **redacción libre** — el Conductor arma el `contenido` él mismo (una minuta o síntesis real y legible, no una transcripción del chat) y lo persiste vía `utils/casos.py::guardar_documento_de_frente` (ya existente, reusado tal cual — no hubo que escribir persistencia nueva) con `agente="conductor"`, mismo tipo `documento_caso` que ya usan los especialistas — así aparece automáticamente en `documentos_producidos_detalle` (Decisión I). Verificado real de punta a punta contra staging (no producción): (1) pedido explícito → documento creado y confirmado leyendo `ver_documento` de vuelta con el contenido exacto; (2) pregunta normal sin pedir guardar → cero tool calls, no lo hizo por su cuenta; (3) insinuación sin confirmar ("estaría bueno que quede documentado, ¿se puede?") → propuso título + resumen de qué guardaría y preguntó "¿Lo guardo así?" sin llamar la tool — los 3 caminos se comportaron exactamente como se diseñó. | 2026-08-19 |
