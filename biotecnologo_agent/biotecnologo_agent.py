...
Tools: search_literature (OpenAlex), buscar_corpus_cientifico (CONICET+INTA),
       ...
       search_chebi (clasificación química/biológica, nuevo),
...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "OpenAlex para temas de microbiología/bioquímica/salud (ej. rutas de degradación,\n"
            "toxicología de un producto candidato). Complementa search_literature, no lo\n"
            "reemplaza — no exhaustiva por diseño (no tiene REQUISITO PARA CERRAR), es una\n"
            "consulta de precisión más, mismo criterio que search_kegg/search_rhea/search_pubchem.\n"
            "Query en inglés."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            ...
            "Sebas aportó (ej. composición de un efluente/sustrato en el tiempo) — media, desvío,\n"
            "CV%, tendencia real (Kendall tau) y clasificación ESTABLE/MODERADAMENTE_VARIABLE/\n"
            "NO_ESTABLE. Usar cuando necesites saber si la materia prima candidata a un bioproceso\n"
            "es consistente en el tiempo, antes de evaluar si un proceso diseñado sobre ella es\n"
            "viable. Nunca calcules esto vos mismo leyendo la tabla — llamá esta tool."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "un solo punto) — ej. si la composición de un sustrato/efluente se movió a un nuevo\n"
            "nivel en algún momento del período, no solo osciló. Un CV% sobre todo el período\n"
            "puede esconder esto (mezcla regímenes distintos en un solo número). Usar cuando\n"
            "sospeches que 'lo típico' cambió durante el período de datos disponible."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Correlación real con desfase temporal entre dos series (ej. composición de la\n"
            "materia prima vs. un parámetro del producto/proceso resultante varias semanas\n"
            "después). SIEMPRE incluye un chequeo de robustez (series diferenciadas) — si la\n"
            "correlación no lo pasa, vuelve marcada 'robusta: false' con una advertencia: es\n"
            "señal de que puede ser una coincidencia de tendencias compartidas, no una relación\n"
            "real. Nunca reportes una correlación como hallazgo real sin mirar ese campo."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
- search_pubmed: literatura biomédica (NCBI, vocabulario MeSH) — más preciso que
  search_literature para temas de microbiología/bioquímica/toxicología de un producto
  candidato. No es exhaustiva por diseño, es una consulta de precisión más (como search_kegg/
  search_rhea/search_pubchem), no tiene mínimo obligatorio.
- analizar_estabilidad_serie / detectar_cambios_de_regimen / correlacion_con_desfase /
  comparar_fuentes_de_datos: cómputo numérico REAL (no estimado a ojo) sobre series de datos
  que Sebas aportó — ej. composición de un sustrato/efluente en el tiempo. Si te dan una tabla
  con fechas y valores y te preguntan si es estable, si cambió de nivel, o si se correlaciona
  con otra variable, extraé los puntos y llamá la tool correspondiente — nunca calcules una
