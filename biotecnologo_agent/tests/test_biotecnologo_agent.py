...
async def test_despachar_tool_leer_serie_de_documento_aportado():
    with (
        patch("biotecnologo_agent.obtener_archivo_original_de_documento_aportado", new=AsyncMock(return_value={"archivo_original_b64": "QUJD", "archivo_nombre": "d.xlsx"})) as mock_get,
        patch("biotecnologo_agent._leer_serie_de_excel_fn", return_value={"serie": [], "n": 0}) as mock_leer,
    ):
        result = await bt._despachar_tool(
    ...
    mock_get.assert_awaited_once_with("doc-1", tenant=bt._TENANT)
...
async def test_despachar_tool_leer_serie_de_documento_aportado_sin_archivo():
    with patch("biotecnologo_agent.obtener_archivo_original_de_documento_aportado", new=AsyncMock(return_value={"error": "no tiene archivo"})):
        result = await bt._despachar_tool(
            "leer_serie_de_documento_aportado",
            {"documento_id": "doc-1", "hoja": "H1", "columna_fecha": "f", "columna_valor": "v"},
            verbose=False,
