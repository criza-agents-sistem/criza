...
| I | Etapa 12 (2026-08-16) — Sebas, mirando el chat recién construido: "¿no puedo hacerles preguntas que no sean en el marco de un caso? me preocupa el consumo de tokens, tal vez necesito hacer una consulta simple antes de abrir un caso nuevo." ¿`frente_id` sigue obligatorio en `enviar_mensaje`, o se vuelve opcional? | Obligatorio (forzar siempre pasar por un caso) / Opcional ("consulta libre" sin caso | **Opcional — `frente_id: str \| None = None`.** Sin `frente_id`: no se llama `obtener_frente_con_caso` (cero queries de más), y `bloque_lecciones_para_prompt` usa el texto de la pregunta como `consulta` en vez de una descripción de caso que no existe — de hecho más preciso, no un downgrade. Con `frente_id`: comportamiento idéntico a la decisión H, sin cambios. Resultado neto: la consulta libre es MÁS barata en tokens que el modo con caso (exactamente lo que Sebas pedía), no una alternativa más cara. | 2026-08-16 |
...
Decisiones A–I cerradas, ninguna abierta. El agente clona un patrón ya probado (evidence_generalista) —
