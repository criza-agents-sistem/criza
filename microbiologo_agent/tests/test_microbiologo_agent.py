...
def test_build_input_desde_frente_incluye_documentos_producidos():
    """Etapa 20, 2026-09-05 — Sebas: 'estaría bueno que todos puedan ver qué está realizando el
    resto'. Solo la lista liviana (título/agente/fecha/id), no el contenido completo."""
    producidos = [
...
async def test_despachar_tool_ver_informe_especialista():
    with patch("microbiologo_agent.obtener_documento_por_id", new=AsyncMock(return_value={"titulo": "t", "contenido": "c"})) as mock_fn:
        result = await ma._despachar_tool("ver_informe_especialista", {"documento_id": "doc-1"}, verbose=False)
...
async def test_despachar_tool_search_pubmed():
    with patch("microbiologo_agent._search_pubmed_fn", return_value={"total_found": 94, "results": []}) as mock_fn:
        result = await ma._despachar_tool("search_pubmed", {"query": "digestate valorization"}, verbose=False)
...
async def test_despachar_tool_analizar_estabilidad_serie():
    ...
    with patch("microbiologo_agent._analizar_estabilidad_fn", return_value={"clasificacion": "ESTABLE"}) as mock_fn:
        result = await ma._despachar_tool("analizar_estabilidad_serie", {"datos": datos}, verbose=False)
...
async def test_despachar_tool_detectar_cambios_de_regimen():
    datos = [{"fecha": "2023-01-01", "valor": 1.0}]
    with patch("microbiologo_agent._detectar_cambios_de_regimen_fn", return_value={"quiebres_detectados": []}) as mock_fn:
        result = await ma._despachar_tool("detectar_cambios_de_regimen", {"datos": datos, "periodo": "W"}, verbose=False)
...
async def test_despachar_tool_correlacion_con_desfase():
    ...
    with patch("microbiologo_agent._correlacion_con_desfase_fn", return_value={"mejor_desfase": {}}) as mock_fn:
        result = await ma._despachar_tool("correlacion_con_desfase", {"serie_x": sx, "serie_y": sy}, verbose=False)
...
async def test_despachar_tool_comparar_fuentes_de_datos():
    ...
    with patch("microbiologo_agent._comparar_fuentes_fn", return_value={"comparables": True}) as mock_fn:
...
async def test_run_agent_desde_frente_captura_submit_y_escribe_token_usage_en_frente():
    ...
    with patch("microbiologo_agent._ai_complete", new=AsyncMock(return_value=mock_response)), \
         ...
         patch("microbiologo_agent.obtener_documentos_de_frente", new=AsyncMock(return_value=[])), \
         patch("microbiologo_agent.motor_api.actualizar_props", new=mock_actualizar), \
         patch("microbiologo_agent.aprendizaje.ensure_area", new=AsyncMock()), \
         patch("microbiologo_agent.run_preflight", new=AsyncMock(return_value=_PREFLIGHT_OK)), \
         patch("microbiologo_agent.aprendizaje.bloque_lecciones_para_prompt", new=AsyncMock(return_value="")):
