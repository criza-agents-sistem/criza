...
def test_tools_count():
    assert len(ia.TOOLS) == 11, f"Esperado 11 tools, tiene {len(ia.TOOLS)}"
...
def test_build_input_desde_frente_incluye_documentos_aportados():
    ...
    result = ia.build_input_desde_frente(FRENTE_TEST, CASO_TEST, [], aportados)
...
def test_build_input_desde_frente_incluye_documentos_producidos():
    ...
    result = ia.build_input_desde_frente(FRENTE_TEST, CASO_TEST, [], None, producidos)
...
async def test_despachar_tool_ver_informe_especialista():
    with patch("ingeniero_ambiental_agent.obtener_documento_por_id", new=AsyncMock(return_value={"titulo": "t", "contenido": "c"})) as mock_fn:
...
async def test_despachar_tool_leer_serie_de_documento_aportado():
    with (
        patch("ingeniero_ambiental_agent.obtener_archivo_original_de_documento_aportado", new=AsyncMock(return_value={"archivo_original_b64": "QUJD", "archivo_nombre": "d.xlsx"})) as mock_get,
        patch("ingeniero_ambiental_agent._leer_serie_de_excel_fn", return_value={"serie": [], "n": 0}) as mock_leer,
    ):
        result = await ia._despachar_tool(
    ...
    mock_get.assert_awaited_once_with("doc-1", tenant=ia._TENANT)
...
async def test_despachar_tool_leer_serie_de_documento_aportado_sin_archivo():
    with patch("ingeniero_ambiental_agent.obtener_archivo_original_de_documento_aportado", new=AsyncMock(return_value={"error": "no tiene archivo"})):
        result = await ia._despachar_tool(
            "leer_serie_de_documento_aportado",
            {"documento_id": "doc-1", "hoja": "H1", "columna_fecha": "f", "columna_valor": "v"},
            verbose=False,
...
async def test_run_agent_desde_frente_captura_submit_y_escribe_token_usage_en_frente():
    ...
    with patch("ingeniero_ambiental_agent._ai_complete", new=AsyncMock(return_value=mock_response)), \
         ...
         patch("ingeniero_ambiental_agent.obtener_documentos_aportados_de_frente", new=AsyncMock(return_value=[])), \
         patch("ingeniero_ambiental_agent.obtener_documentos_de_frente", new=AsyncMock(return_value=[])), \
